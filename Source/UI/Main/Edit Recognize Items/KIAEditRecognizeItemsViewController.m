...
- (IBAction)addToMyKitchIn:(id)sender
{
    if ([[NSUserDefaults standardUserDefaults] objectForKey:@"sessionId"])
    {
        ...
        for (int i = 0; i < [_itemArray count]; i++)
        {
            ...
            {
                [[KIAUpdater sharedUpdater] addItemFromKitchInWihtId:[item Id] name:[item ItemName] categoryId:[[_category objectForKey:[[item Category] uppercaseString]] integerValue] shortName:[item ItemShortName] count:1 value:@"1"];
